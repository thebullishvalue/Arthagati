"""
Arthagati — Shared CSS, chart theming, and colour constants for the UI layer.
अर्थगति (Arthagati) — "Movement of meaning / market sentiment"

Ported from Tattva's UI layer. The architecture is adopted whole because it
solves problems this app also has and had solved worse: a light theme that is
a token swap rather than a second stylesheet, a chart palette resolved at
RENDER time (so plots flip with the chrome instead of keeping the colours they
were handed at import), a painted Plotly canvas (so a device's system theme
cannot bleed into the plot area), and one Plotly config so no chart ships
Plotly's own toolbar.
...
# ── The dark ramp, as Python ────────────────────────────────────────────────
# Mirrors the :root block in theme.css. Plotly cannot read CSS custom
# properties, so the palette has to exist twice; these are the authoritative
# Python copies and `_PALETTE_RGB` below is what chart code resolves through.
COLOR_ACCENT = "#4C7DF0"   # Cobalt — interactive / primary
COLOR_GREEN  = "#2CA36B"   # long / positive
COLOR_RED    = "#DD5A5A"   # short / negative
COLOR_AMBER  = "#D79A3C"   # caution ONLY — never brand
COLOR_CYAN   = "#4E9FC4"   # informational
COLOR_PURPLE = "#8B72E0"   # secondary / attribution
COLOR_SLATE  = "#7E8797"   # neutral / muted
...
# `_PALETTE_RGB` above is a single palette tuned for the dark ground, and
...
def chart_rgba(name: str, alpha) -> str:
    ...
    For fills and bands, where a hex cannot carry the alpha.
    """
    r, g, b = _palette()[name]
...
PLOTLY_CONFIG = dict(
    ...
    toImageButtonOptions=dict(format="png", scale=2, filename="arthagati-chart"),
    scrollZoom=False,
    doubleClick="reset",
    responsive=True,
)
...
def chart_layout(
    ...
    """Return a base Plotly layout dict for the active appearance.
...
def inject_css(theme: str = "dark") -> None:
    """Inject the Arthagati design system into the Streamlit app.
...
# console prints (`console.start_phase(name, n, 4)` in arthagati.py). Before this the
...
# ends at 24 and not 25 — 25 is the correlation band's first number.
RUN_PHASES = (
    (1, 0, 24, "Data Ingestion"),
    (2, 25, 44, "Correlation Engine"),
    (3, 45, 79, "Sentiment Engine"),
    (4, 80, 100, "MSF Spread & WaveTrend"),
)
...
def apply_chart_theme(fig) -> None:
    """Apply the active appearance to a Plotly figure (mutates in place)."""
